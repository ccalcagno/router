...
abstract class Dispatch
{
   ...
   * @return Dispatch
   */
  public function group(?string $group, null|array|string $middleware = null): Dispatch
  {
  ...
  /**
   * Retorna a URL base dos assets ou de um asset específico.
   *
   * @param string $path Caminho relativo do asset (opcional).
   * @return string
   */
  public function assets(string $path = ''): string
  {
    $base = rtrim($this->projectUrl, '/') . '/assets';
    $path = ltrim($path, '/');
  ...
  /**
   * Gera uma URL absoluta para um recurso específico.
   *
   * @param string $path
   * @return string
   */
